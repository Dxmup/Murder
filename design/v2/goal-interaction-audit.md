# V2 Goal and Interaction Audit 01

## Rules applied

- Every main goal has an observable success condition.
- No goal requires identifying R, proving murder, or establishing that the article existed.
- Every character must seek people in at least three social knots across the evening.
- Disclosure is voluntary and usually conditional on protection, recognition, access, money, correction, or reciprocal information.
- A character's final choice can remain difficult under all four endings.
- Abilities, where later added, cannot replace the listed interaction.

## Interaction coverage

The matrix gives each core character three waves of named targets:

- **Opening:** establish existing relationships and immediate non-mystery wants.
- **Midgame:** exchange or test identity evidence.
- **Endgame:** confront Vale evidence, make commitments, and choose what becomes public.

Repeated pairings are intentional when their relationship evolves. Booklets should never instruct a character merely to “talk to everyone” or “solve the mystery.”

## High-risk roles

- **Arden:** host access cannot compel participation or turn into GM authority.
- **Frankie:** charisma and age cannot authenticate memory.
- **Mira:** money cannot purchase another player's agreement or count as a completed goal without substantive terms.
- **Eli:** technical explanation must be distributed and bounded.
- **Edi:** a synthetic-media demonstration establishes possibility, not historical use.
- **Celeste:** introductions require concessions so the broker cannot move frictionlessly.
- **Morgan:** a Case Conference is an invitation to consenting players, not testimony mechanics.
- **Kit:** completion requires corrections and independent corroboration, not volume or audience reach.

## Quiet-role protections

- Maren's worker commitments remain valuable if every R theory is wrong.
- Rafi controls a debt and neighborhood account rather than serving as a passive witness.
- Dee negotiates protection before resolving the access gap.
- Samira decides the ethical boundary around a missing partner's private life.
- Inez converts institutional need for local context into commitments.

## Directed graph result

The first graph audit passes for the 16-player base cast:

- every core role has at least four distinct outgoing targets when optional characters are removed;
- every core role has at least four other characters actively seeking them;
- outgoing range is 4–8 and incoming range is 4–8;
- Morgan has the highest outgoing count (8), while Eli, Samira, Kit, and Morgan have the highest incoming pressure (7–8).

Those counts establish access, not quality. Eli and Morgan remain potential funnels, and Kit remains a potential attention sink.

## Next tests

1. Check that no pair is connected only through mystery exposition.
2. Draft one opening test scene per character to see whether goals produce action before any message arrives.
3. Define what evidence or commitment changes hands in each successful bargain.
4. Only then draft the three-section message schedule.
