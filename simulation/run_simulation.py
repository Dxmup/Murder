#!/usr/bin/env python3
"""Run the deterministic, isolated-context social mystery simulation."""
...
def main() -> int:
    ...
    briefs = {}
    for cid in roster:
        matches = list(BRIEFS.glob(f"{cid}-*.md"))
        if len(matches) != 1:
            raise RuntimeError(f"expected one character booklet for {cid}, found {len(matches)}")
        briefs[cid] = matches[0].read_text(encoding="utf-8")
    if len(roster) != 16 or any(not value for value in known.values()):
        raise RuntimeError("The simulation requires 16 core roles, each with at least one initial fact")
    for row in messages:
    ...
    transcript = {"run_id": args.run_id, "protocol": "isolated 16-agent three-section simulation", "sections": phases}
